...
exports.getInputs = (ctx, initialConfigurationState) => {
  ...
  if (ctx.query.returnTo) {
    ...
    return [
      { configurationState: initialConfigurationState, returnTo: ctx.query.returnTo, returnToState: ctx.query.state },
      data,
    ];
    ...
    if (issuerCreated) {
      try {
        await Superagent.delete(
          `${ctx.body.baseUrl}/v1/account/${ctx.body.accountId}/issuer/${encodeURIComponent(issuerId)}`
        ).set('Authorization', ctx.headers['authorization']); // pass-through authorization
      } catch (_) {}
    }
    throw e;
