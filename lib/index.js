...
exports.createStorage = async (ctx, useFunctionAccessToken) => {

    if (useFunctionAccessToken) {
        if (!ctx.fusebit.functionAccessToken) {
            throw new Error('Creating storage context using a function access token requires that the caller is also in posession of the function access token.')
        }
        const storageCtx = {
            fusebit_storage_id: `boundary/${ctx.boundaryId}/function/${ctx.functionId}/${uuid.v4()}`,
        ...
        try {
            await Superagent.get(`${exports.generateStorageUrl(storageCtx)}/*?count=1`)
                .set('Authorization', `Bearer ${ctx.fusebit.functionAccessToken}`);
        }
        catch (e) {
            throw new Error(`Creating storage context that uses a function access token requires that the caller itself is in posession of a function access token with permissions to ${generateStoragePath(storageCtx)} storage id.`);
        }
        return storageCtx;
    }
...
exports.getStorageClient = (ctx) => {
    ...
    let accessToken = ctx.fuesbit.functionAccessToken;
